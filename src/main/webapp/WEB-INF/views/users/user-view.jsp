	...
	<div class="section">
		...
		<div class="main">
			<div class="container-fluid">
				...
				<div class="action">
					<b class="text-danger">${user.firstName} ${empty user.lastName ? '' : ' '.concat(user.lastName)}</b>
					<div class="action-btn">
						<button type="button" class="btn btn-outline-primary">
							<i class="bi bi-printer"></i>Print
						</button>
						<a href="${pageContext.request.contextPath}/users/edit/${user.id}"><button
					...
					<div class="p-table1">
						<table class="table table-bordered border-light">
							<tr class="table-light">
								<td>Name</td>
								<td>${user.firstName} ${empty user.lastName ? '' : ' '.concat(user.lastName)}</td>
							</tr>
							<tr class="table-light">
								<td>Username</td>
								<td>${user.username}</td>
								...
								<td>
									<c:choose>
										<c:when test="${!u.isAccountNonLocked()}">
											<span class="badge rounded-pill bg-primary">Non-locked</span>
										</c:when>
										<c:otherwise>
